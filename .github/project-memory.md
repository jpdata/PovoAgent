...
## Project Cache for New Projects (2026-09-10)

- Fixed: `PROJECT_CACHE.md` was only generated by `analysis` Mode 2 (Assessment), so new projects never produced a cache.
- All five scaffold skills (`flutter-scaffold`, `react-scaffold`, `angular-scaffold`, `dotnet-scaffold`, `astro-scaffold`) now generate the initial `PROJECT_CACHE.md` at the end of the Scaffold phase.
- `implementation`, `testing`, and `review` skills now have an "Update Project Cache" step (refresh log + symbol index + file index).
- `analysis` Mode 1 has a note clarifying the cache is generated at Scaffold, not during Analysis.
- Agent templates (povo, povo-direct, codex, opencode) updated: Cache Lifecycle table + New Project workflow generate the cache after Scaffold.
