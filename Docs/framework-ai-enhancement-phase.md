...
### 2026-09-19 - PovoAgent always analyzes and plans before execution

- Clarified that `PovoAgent` must always perform **Analyze** and **Plan** before executing any request, while retaining the execution capabilities available in `PovoDirect` after those steps.
- Preserved `PovoDirect` as the agent for immediate execution when the mandatory analysis and planning sequence is not desired.
- Updated the three `PovoAgent` templates for the default, Codex, and OpenCode platforms.
- Why: the main agent must consistently provide analysis and planning as its primary behavior while still supporting the direct execution workflow after that gate.
- Affected files: `templates/povo.agent.md`, `templates/codex/povo.agent.md`, `templates/opencode/povo.agent.md`, `Docs/framework-ai-enhancement-phase.md`.

### 2026-09-19 - Specification-first lifecycle alignment

- Added `Specification` as a mandatory phase between `Design` and `Scaffold` in the main PovoAgent lifecycle for the default, Codex, and OpenCode templates.
- Updated lifecycle gates so Implementation consumes approved `SPEC_<Feature>.md` documents, Testing covers their acceptance criteria, and Review validates spec conformance.
- Removed the Vertical Slice Architecture exception that allowed Design to move directly to Implementation.
- Updated existing-project modification guidance so behavior changes update or create a specification before implementation; contract or architecture changes add Design before Specification.
- Why: the framework already had a complete Specification skill and SDD documentation, but the main agent lifecycle and modification rules were not consistently enforcing them.
- Affected files: `templates/povo.agent.md`, `templates/codex/povo.agent.md`, `templates/opencode/povo.agent.md`, `skills/design/SKILL.md`, `skills/change-intake/SKILL.md`, `Docs/evolutionary-lifecycle.md`, `Docs/framework-ai-enhancement-phase.md`, `.github/project-memory.md`.

### 2026-09-10 - Project Cache generated for new projects at Scaffold

- Fixed: `PROJECT_CACHE.md` was only generated by `analysis` Mode 2 (Existing Project Assessment), so new projects never produced a cache.
- Added a **"Generate Initial Project Cache"** final step to all five scaffold skills (`flutter-scaffold`, `react-scaffold`, `angular-scaffold`, `dotnet-scaffold`, `astro-scaffold`) — the cache is now created at the end of the Scaffold phase, the earliest point where the project structure exists.
- Added an **"Update Project Cache"** step to `skills/implementation/SKILL.md`, `skills/testing/SKILL.md`, and `skills/review/SKILL.md` so subsequent phases keep the cache fresh (refresh log + symbol index + file index).
- Added a **"Mode 1 — Project Cache"** note to `skills/analysis/SKILL.md` clarifying that new-project caches are generated at Scaffold, not during Analysis.
- Updated agent templates (`templates/povo.agent.md`, `templates/povo-direct.agent.md`, `templates/codex/povo.agent.md`, `templates/codex/povo-direct.agent.md`, `templates/opencode/povo.agent.md`, `templates/opencode/povo-direct.agent.md`) — the Cache Lifecycle table and the New Project workflow now generate the initial cache after Scaffold.
- Updated `Docs/project-cache-system.md` and `Docs/new-project-lifecycle.md` to reflect the new creation point.
- Why: new projects never reached the Assessment phase, so the cache that all skills rely on to avoid redundant scans was never created. Generating it at Scaffold makes it available from the first Implementation task onward.
- Affected files: `flutter/skills/flutter-scaffold/SKILL.md`, `react/skills/react-scaffold/SKILL.md`, `angular/skills/angular-scaffold/SKILL.md`, `dotnet/skills/dotnet-scaffold/SKILL.md`, `astro/skills/astro-scaffold/SKILL.md`, `skills/implementation/SKILL.md`, `skills/testing/SKILL.md`, `skills/review/SKILL.md`, `skills/analysis/SKILL.md`, `templates/povo.agent.md`, `templates/povo-direct.agent.md`, `templates/codex/povo.agent.md`, `templates/codex/povo-direct.agent.md`, `templates/opencode/povo.agent.md`, `templates/opencode/povo-direct.agent.md`, `Docs/project-cache-system.md`, `Docs/new-project-lifecycle.md`, `Docs/framework-ai-enhancement-phase.md`, `VERSION`, `.github/project-memory.md`.
